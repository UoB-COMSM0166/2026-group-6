 ...
 *   - onPlayerContact() 钩子, 子类覆写实现各种交互
 *
 * 继承示例:
 *   class Checkpoint extends Entity {
 *      onPlayerContact(player, gm) { gm.setCheckpoint(this); }
 *   }
 *
 *   class Collectible extends Entity {
 *      onPlayerContact(player, gm) { player.coins++; this.destroy(); }
 *   }
 */
class Entity {
   /**
   ...
   constructor(x, y, w, h, spawnData = {}) {
      ...
      this.displayColor = spawnData.color || '#FF00FF';
    ...
    *
    * 示例:
    *   class Spike    → onPlayerContact() { player.hp -= 1; }
    *   class Coin     → onPlayerContact() { score++; this.destroy(); }
    *   class Portal   → onPlayerContact() { gm.teleport(...); }
    *
    * @param {Player} player
